...
class TestConfirm:
    @pytest.mark.asyncio
    async def test_saves_expense_and_edits_message(self, with_pending, mocker):
        ...
        assert "45,90" in mock_edit.call_args[0][2]
        assert "Alimentação" in mock_edit.call_args[0][2]
    ...
    @pytest.mark.asyncio
    async def test_db_error_edits_error_message(self, with_pending, mocker):
        ...
        assert "Erro" in mock_edit.call_args[0][2]
        assert pending_store.get(CHAT_ID) is None
